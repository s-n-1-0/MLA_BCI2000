...
details.overview = (
    "EEGNetを使用し、マルチチャネルEEGデータ(左手・右手の運動イメージ)を2値分類します。\n"
...
parameters.model_architecture="EEGNet"
parameters.input_format= "生波形を入力しています。形式は3階テンソル(バッチサイズを含むと4階)です。750サンプルはランダム区間で選択"
parameters.input_format_map = [mct.KeyVal("1","16ch"),
                               mct.KeyVal("2","750 samples"),
                               mct.KeyVal("3","1")]
...
#評価
#
model_card.quantitative_analysis.graphics.description = "正解率(Accuracy),損失(Loss),再現率(Recall),特異度(Specificity)。85エポックのモデルを選択。"
model_card.quantitative_analysis.graphics.collection =[mct.Graphic("",h64) for h64 in history_base64_list]

# %%
utils.save_and_display("../../docs/model_cards/decoder1.html",toolkit,model_card)